...
class HomeView(AdminLoginRequired, View):
    ...
    def get(self, request):
        context = {}
        posts = Post.objects.filter(type="news").order_by("-id")[:10]
        context["posts"] = posts
...
class CommentsView(AdminLoginRequired, View):
    ...
    def get(self, request):
        ...
        if "disable" in request.GET:
            ...
            comment = get_object_or_404(Comment, pk=pk, post__type="news")
            comment.is_active = False
            comment.save()
            redirect(reverse("news:comments"))
        ...
        posts = Paginator(Comment.objects.filter(post__type="news").order_by("-id"), 30)
...
class PostsView(AdminLoginRequired, View):
    ...
    def get(self, request):
        ...
        posts = Paginator(Post.objects.filter(type="news").order_by("-id"), 30)
        page_number = request.GET.get("page")
        posts_list = posts.get_page(page_number)
...
class AddAuthorView(AdminLoginRequired, View):
    template_name = "news/add-author.html"
    def get(self, request):
        context = {}
        return render(request, self.template_name, context)
...
class EditAuthorView(AdminLoginRequired, View):
    template_name = "news/edit-author.html"
...
class AddPostView(AdminLoginRequired, View):
    template_name = "news/add-post.html"
    def get(self, request):
    ...
    def post(self, request):
        ...
        post = Post.objects.create(slug=slug, title=title, content=content, type="news", is_published=is_published, author=request.user)
...
class EditPostView(AdminLoginRequired, View):
    template_name = "news/edit-post.html"
